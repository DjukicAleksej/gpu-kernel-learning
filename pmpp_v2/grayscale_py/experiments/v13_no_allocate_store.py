...
"""v13: add an L1 no-allocation cache hint to v10's float4 output stores."""
...
__device__ __forceinline__ void store_l1_no_allocate(
    float4* destination,
    const float4& value) {
    asm volatile(
...
__global__ void grayscale_exact_grid_kernel(
    ...
    store_l1_no_allocate(output + group, grayscale_group(image, group));
}
...
_module = load_inline(
    name="pmpp_grayscale_v13_l1_no_allocate_store_ext",
