...
"""v14: retain overlapping input sectors in L1 until their final vector load."""
...
__device__ __forceinline__ float4 load_l1_retain(
    const float4* source) {
    float4 value;
    asm volatile(
...
__device__ __forceinline__ float4 load_l1_release(
    const float4* source) {
    float4 value;
    asm volatile(
...
__device__ __forceinline__ float4 grayscale_group(
    ...
    const float4 values0 = load_l1_retain(values);
    const float4 values1 = load_l1_retain(values + 1);
    const float4 values2 = load_l1_release(values + 2);
...
_module = load_inline(
    name="pmpp_grayscale_v14_input_retention_ext",
