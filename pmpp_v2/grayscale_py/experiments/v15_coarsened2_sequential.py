...
"""v15: store each result before starting the second coarsened group."""
...
CPP_SOURCE = r"""
torch::Tensor launch_grayscale_coarsened2_sequential(
    torch::Tensor image,
    torch::Tensor output);
"""
...
__global__ void grayscale_coarsened2_sequential_kernel(
    const float4* __restrict__ image,
    float4* __restrict__ output) {
    const int first_group = (blockIdx.x << 8) + threadIdx.x;
    output[first_group] = grayscale_group(image, first_group);

    const int second_group = first_group + 128;
    output[second_group] = grayscale_group(image, second_group);
}
...
torch::Tensor launch_grayscale_coarsened2_sequential(
    ...
    if ((pixel_groups & (groups_per_block - 1)) == 0) {
        grayscale_coarsened2_sequential_kernel<<<
            pixel_groups >> 8, threads>>>(input_ptr, output_ptr);
    } else {
    ...
    TORCH_CHECK(error == cudaSuccess,
                "grayscale sequential coarsened-by-two kernel launch failed: ",
...
_module = load_inline(
    name="pmpp_grayscale_v15_coarsened2_sequential_ext",
    cpp_sources=CPP_SOURCE,
    cuda_sources=CUDA_SOURCE,
    functions=["launch_grayscale_coarsened2_sequential"],
...
def custom_kernel(data: input_t) -> output_t:
    image, output = data
    return _module.launch_grayscale_coarsened2_sequential(image, output)
